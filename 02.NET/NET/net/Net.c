...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	strcpy(buf, ",;");
	memset(text, 0, sizeof(text));
	sprintf(text, "prince,%.1f;", Data_init.add_price); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	
	Data_init.add_price = 0;
	Data_init.price_copy = 0;
#elif NET_SERVE == 1
	memset(text, 0, sizeof(text));
	strcpy(buf, "{");
	strcat(buf, text);
...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	//有效数据
	memset(text, 0, sizeof(text));
	sprintf(text, "\"wight\":\"%.1f\",",Data_init.git_wight); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"apple_p\":\"%.1f\",",device_state_init.apple_p); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"orange_p\":\"%.1f\",", device_state_init.orange_p); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"banana_p\":\"%.1f\"",device_state_init.banana_p); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "}");
	strcat(buf, text);
